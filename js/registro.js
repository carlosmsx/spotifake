...
import {
  ...
  funclave,
  validarClave,
  limpiarClaveValidada,
} from "./validaciones.js";
import { login, cambioNav, cerrarSesion } from "./login.js";

let vectorUsuarios =
  JSON.parse(localStorage.getItem("vectorUsuariosKey")) || [];
...
nombre.addEventListener("blur", () => {
  cantidadCaracteres(nombre, 5, 30);
});
...
fechaNacimiento.addEventListener("keyDown", () => {
...
genero.addEventListener("keyDown", () => {
  campoRequerido(genero);
});
calle.addEventListener("blur", () => {
  validarSoloLetras(calle, 4, 20);
});
nroCalle.addEventListener("blur", () => {
...
clave2.addEventListener("blur", () => {
  validarClave(clave, clave2);
});
clave2.addEventListener("focus", () => {
  limpiarClaveValidada(clave2);
});
...
formulario.addEventListener("submit", (e) => {
  ...
  if (validacionGeneral() == false) return;
  ...
  Swal.fire(
    ...
    "Los datos de usuario se registraron correctamente",
    "success"
  );
});
...
function validarUsuarioRepetido(input) {
  let usuarioRepetido = vectorUsuarios.find((item) => {
    return item.usuario == input.value;
  });
